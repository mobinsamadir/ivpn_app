...
# --- 4. CMAKE GENERATION ---
Write-Host ">>> 4. Generating Build Files (Force VS 2026 + x64)..." -ForegroundColor Yellow
if (!(Test-Path "build\windows\x64")) {
    New-Item -ItemType Directory -Force -Path "build\windows\x64" | Out-Null
}
Set-Location -Path "windows"
$cmakeResult = & $CMAKE_EXE -S . -B ..\build\windows\x64 -G "Visual Studio 18 2026" -A x64
if ($LASTEXITCODE -ne 0) {
...
if (!(Test-Path $EXE_PATH)) {
    ...
    Write-Host "🔍 Looking for executable in build directory..." -ForegroundColor Yellow
    ...
    if (!(Test-Path $EXE_PATH)) {
        Write-Host "❌ No executable found in any expected location!" -ForegroundColor Red
        Read-Host "Press Enter to exit..."
        exit 1
    }
}
