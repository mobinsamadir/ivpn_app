...
class SmartPinger {
  ...
  static Future<SmartPingResult> pingMultiple({
    ...
    AdvancedLogger.info('[SmartPing] Starting multi-endpoint ping test (Endpoints: ${endpoints.length})');
    ...
    if (successful.length >= requiredSuccesses) {
      overallSuccess = true;
      recommendation = '✅ Network Healthy - ${successful.length}/${endpoints.length} endpoints responded';
    } else if (successful.isNotEmpty) {
      overallSuccess = true; // Still a success if at least one responded, though we might flag it
      recommendation = '⚠️ Weak Network - Only ${successful.length}/${endpoints.length} endpoints responded';
    } else {
    ...
    final avg = successful.isNotEmpty 
        ? successful.map((r) => r.latency).reduce((a, b) => a + b) / successful.length
        : -1.0;
  ...
  static Future<PingResult> _pingWithRetry(
    String endpoint,
    CancelToken? cancelToken,
    {int maxRetries = 2, required Duration timeout}
  ) async {
