...
### [تاریخ: 2026-08-05] Hunter Report (Phase 1 & 2 Golden Rule)
**اقدامات انجام شده:**
طبق قانون طلایی (Golden Rule)، بدون انتظار برای تأیید انسان فاز اول و دوم را بررسی و اجرا کردم. با بررسی امنیتی فایل‌های مسیر `lib/services/` مشکلات نشت فایل (Information Disclosure) زیر شناسایی و برطرف شدند:
۱. در `windows_vpn_service.dart`: متغیر `_currentConfigPath` هنگام ساخت فایل `config.json` مقداردهی نمی‌شد که باعث می‌شد فایل در متد `stopVpn` پاک نشود. با افزودن `_currentConfigPath = configFile.path;` این باگ امنیتی مرتفع گردید.
۲. در `native_vpn_service.dart`: فایل‌های موقت کانفیگ (`vpn_config_...`، `ping_config_...` و `test_proxy_...`) تنها در بلوک `catch` حذف می‌شدند و در صورت موفقیت‌آمیز بودن ارتباط Native، فایل‌ها روی سیستم باقی می‌ماندند. با انتقال `deleteSync` به بلوک `finally`، این نشت اطلاعات به طور کامل و برای همیشه رفع شد.
تمامی تغییرات با موفقیت تست و روی پروژه اعمال گردید. آماده کامیت هستم.
