...
void main() {
  ...
  group('ConfigManager Core Logic Shield', () {
    test(
        'Scenario 1: Smart Failover Resilience - Fails twice, succeeds on third',
        () async {
      // Setup: Add 3 configs with decreasing timestamps so c1 is picked first (Sort: Date Descending)
      final now = DateTime.now();
      final c1 = VpnConfigWithMetrics(
          id: 'c1',
          rawConfig: 'vless://c1',
          name: 'Config 1',
          addedDate: now.add(const Duration(minutes: 3)),
          funnelStage: 2);
      final c2 = VpnConfigWithMetrics(
          id: 'c2',
          rawConfig: 'vless://c2',
          name: 'Config 2',
          addedDate: now.add(const Duration(minutes: 2)),
          funnelStage: 2);
      final c3 = VpnConfigWithMetrics(
          id: 'c3',
          rawConfig: 'vless://c3',
          name: 'Config 3',
          addedDate: now.add(const Duration(minutes: 1)),
          funnelStage: 2);
      ...
      when(() => mockTester.runTest(any(), mode: any(named: 'mode')))
        ...
        final config =
            invocation.positionalArguments[0] as VpnConfigWithMetrics;
      ...
      when(() => mockNativeService.connect(c3.rawConfig))
          .thenAnswer((_) async {});
      ...
      // Verify
      verify(() => mockTester.runTest(
          any(that: predicate((c) => (c as VpnConfigWithMetrics).id == 'c1')),
          mode: any(named: 'mode'))).called(1);
      verify(() => mockTester.runTest(
          any(that: predicate((c) => (c as VpnConfigWithMetrics).id == 'c2')),
          mode: any(named: 'mode'))).called(1);
      verify(() => mockTester.runTest(
          any(that: predicate((c) => (c as VpnConfigWithMetrics).id == 'c3')),
          mode: any(named: 'mode'))).called(1);
    ...
    test('Scenario 2: Total Blackout - All configs fail', () async {
      ...
      verify(() => mockTester.runTest(any(), mode: any(named: 'mode')))
