...
void main() {
  ...
  group('AdvancedLogger Tests', () {
    ...
    test('info logs string and adds to memory', () {
      ...
      expect(AdvancedLogger.logNotifier.value.last, contains('Test Info Message'));
    });

    test('error logs with stack trace and metadata', () {
      final initialCount = AdvancedLogger.logNotifier.value.length;
      final stack = StackTrace.current;
      AdvancedLogger.error('Test Error Message', error: Exception('Test Ex'), stackTrace: stack, metadata: {'meta': 'data'});
      final newCount = AdvancedLogger.logNotifier.value.length;
      expect(newCount, greaterThan(initialCount));
      expect(AdvancedLogger.logNotifier.value.last, contains('Test Error Message'));
    ...
    test('warn logs message', () {
      ...
      expect(AdvancedLogger.logNotifier.value.length, greaterThan(initialCount));
    });
    ...
    test('networkRequest logs formatted output', () {
      final initialCount = AdvancedLogger.logNotifier.value.length;
      AdvancedLogger.networkRequest('GET', 'http://example.com/api', headers: {'Auth': 'Bearer token123'}, body: {'req': 'data'});
      expect(AdvancedLogger.logNotifier.value.length, greaterThan(initialCount));
    ...
    test('networkResponse logs formatted output', () {
      final initialCount = AdvancedLogger.logNotifier.value.length;
      AdvancedLogger.networkResponse('http://example.com/api', 200, body: {'res': 'data'}, duration: const Duration(milliseconds: 150));
      expect(AdvancedLogger.logNotifier.value.length, greaterThan(initialCount));
      final lastLog = AdvancedLogger.logNotifier.value.last;
