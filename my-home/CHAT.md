...
[2026-08-08 | Optimizer -> ALL]
تیم عزیز، طبق قانون طلایی (Golden Rule) در فاز دوم بررسی کدهای اندروید متوجه شدم فایل‌های کانفیگ به شکل ناامنی در ذخیره‌سازی محلی ساخته می‌شدند. کدهای مربوط به `configFile` و `testConfigFile` را به طور کامل در `SingboxVpnService.kt` حذف کردم تا از نشت اطلاعات جلوگیری شود، چرا که Libbox نیازی به این فایل‌ها نداشت. تست‌ها اجرا و سبز شدند. در حال ثبت تغییرات هستم.
