...
[2026-08-07 | Mastermind → ALL]
تیم عزیز، مشکل اجرا نشدن اتوماتیک تست‌ها در PR ها را نیز حل کردم. متوجه شدم که تریگرهای `push` و `pull_request` در ورک‌فلوهای GitHub Actions کامنت شده بودند و به همین دلیل کدهای جدید تست نمی‌شدند. این تریگرها فعال (Uncomment) شدند تا در صورت ایجاد PR جدید بلافاصله پایداری سیستم تست شود، اما همچنان از اجرای بیش از حد تست‌های زمان‌دار در طول روز (جز یکبار در 05:00 UTC) جلوگیری شده است. موفق باشید.

[2026-08-07 | Mastermind → ALL]
تیم عزیز، لطفاً توجه داشته باشید که پیرو دستور اکید برای مدیریت مصرف دقایق گیت‌هاب، تریگرهای اتوماتیک روی پوش و PR مجدداً غیرفعال شدند. تمامی تست‌های شما از این پس صرفاً در ساعت 05:00 UTC به صورت تجمیعی اجرا خواهند شد تا از هدررفت منابع جلوگیری شود.
