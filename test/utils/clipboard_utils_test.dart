  ...
  group('ClipboardUtils', () {
    group('detectFormat', () {
      test('should correctly identify standard formats', () {
        expect(
            ClipboardUtils.detectFormat('vmess://something'), equals('vmess'));
        expect(
            ClipboardUtils.detectFormat('vless://something'), equals('vless'));
        expect(ClipboardUtils.detectFormat('ss://something'),
            equals('shadowsocks'));
        expect(ClipboardUtils.detectFormat('trojan://something'),
            equals('trojan'));
      });

      test('should correctly identify subscriptions and URLs', () {
        expect(ClipboardUtils.detectFormat('https://example.com/subscribe'),
            equals('subscription'));
        expect(ClipboardUtils.detectFormat('http://example.com/sub/user'),
            equals('subscription'));
        expect(ClipboardUtils.detectFormat('https://example.com/something'),
            equals('url'));
        expect(
            ClipboardUtils.detectFormat('http://example.com'), equals('url'));
      });

      test('should be resilient to whitespace', () {
        expect(ClipboardUtils.detectFormat('  vmess://something  '),
            equals('vmess'));
      ...
      test('should default to unknown for empty or malformed strings', () {
        ...
        expect(ClipboardUtils.detectFormat('   '), equals('unknown'));
        expect(ClipboardUtils.detectFormat('unknown://something'),
            equals('unknown'));
        expect(ClipboardUtils.detectFormat('just_some_random_text'),
            equals('unknown'));
      });
    ...
    group('validateConfig', () {
      test('validates valid vmess configs', () {
        ...
        expect(ClipboardUtils.validateConfig('vmess://$validBase64'), isTrue,
      ...
      test('invalidates vmess with invalid base64 content', () {
        expect(
            ClipboardUtils.validateConfig(
                'vmess://not-a-valid-base64-string!@#'),
            isFalse,
      ...
      test('validates valid vless configs', () {
        expect(ClipboardUtils.validateConfig('vless://uuid@host:port'), isTrue,
      ...
      test('validates valid shadowsocks configs', () {
        expect(ClipboardUtils.validateConfig('ss://method:password@host:port'),
            isTrue,
      ...
      test('validates valid trojan configs', () {
        expect(ClipboardUtils.validateConfig('trojan://password@host:port'),
            isTrue,
      ...
      test('trims whitespace and validates valid configs correctly', () {
        ...
        expect(
            ClipboardUtils.validateConfig('   vmess://$validBase64   '), isTrue,
